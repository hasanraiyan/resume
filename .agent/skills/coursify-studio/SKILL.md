...
### Bundled CLI Tool: `manage_content.cjs`

The skill includes a deterministic CLI script to manage content directly. Use it via `node` from the workspace root:

```bash
# List all courses
node .agent/skills/coursify-studio/scripts/manage_content.cjs list-courses

# Get full course content
node .agent/skills/coursify-studio/scripts/manage_content.cjs get-course <id>

# Upsert a section (JSON payload)
node .agent/skills/coursify-studio/scripts/manage_content.cjs upsert-section '{"courseId":"...", "title":"New Lesson", "status":"complete"}'
```
