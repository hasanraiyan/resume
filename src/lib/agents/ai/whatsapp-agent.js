...
class WhatsAppAgent extends BaseAgent {
  ...
  async _onInitialize() {
    this.logger.info('WhatsApp Assistant initialized');
  }

  async _validateInput(input) {
    if (!input || !input.userMessage) {
      throw new Error('userMessage is required for WhatsApp Assistant');
    }
  }
  ...
  async *_onStreamExecute(input) {
    ...
    if (assistantContent?.trim()) {
      new Analytics({
        eventType: 'whatsapp_interaction',
        path: 'whatsapp',
        sessionId,
  ...
  async _onExecute(input) {
    throw new Error('WhatsAppAgent only supports streamExecute');
  }
}
