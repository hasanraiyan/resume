  ...
  try {
    ...
    if (!slideImageBase64 || !editPrompt) {
      return NextResponse.json(
        { error: 'slideImageBase64 and editPrompt are required' },
        { status: 400 }
      );
    }
    ...
    if (slideImageBase64.startsWith('http')) {
      try {
        ...
        return NextResponse.json(
          { error: 'Failed to process the input image URL' },
          { status: 400 }
        );
      }
    } else {
      // The image editor agent takes raw base64 (without the data:image/png;base64, prefix)
      rawBase64 = slideImageBase64.includes('base64,')
        ? slideImageBase64.split('base64,')[1]
        : slideImageBase64;
    }
    ...
    return NextResponse.json({ error: error.message || 'Failed to edit slide' }, { status: 500 });
  }
}
