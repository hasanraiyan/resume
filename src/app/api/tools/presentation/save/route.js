  ...
  try {
    ...
    if (!isAdmin) {
      return NextResponse.json(
        { error: 'Unauthorized: Admin access required to save to database.' },
        { status: 403 }
      );
    }
    ...
    if (!presentationId || !slides || !Array.isArray(slides)) {
      return NextResponse.json(
        { error: 'Invalid payload: presentationId and an array of slides are required.' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to save presentation' },
      { status: 500 }
    );
  }
}
