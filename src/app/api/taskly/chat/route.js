...
export async function POST(request) {
  ...
  try {
    ...
    const sessionId = `taskly-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        const safeError = (error) => {
          ...
          try {
            ...
            if (!isClosedStreamError(streamError)) {
              console.error('[Taskly Chat] Failed to propagate stream error:', streamError);
            }
          } finally {
            isClosed = true;
          }
        ...
        try {
          const events = agentRegistry.streamExecute(AGENT_IDS.TASKLY_ASSISTANT, inputParams);
          ...
          console.error('[Taskly Chat] Stream error:', error);
          safeError(error);
        } finally {
    ...
    console.error('[Taskly Chat] Fatal error:', error);
