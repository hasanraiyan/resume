        ...
        {loading ? (
          ...
          <div className="text-center py-24 bg-neutral-50 border-2 border-dashed border-neutral-200 rounded-3xl">
            ...
            <p className="text-neutral-500 max-w-md mx-auto">
              There are no generated presentations matching your criteria. Try creating one using
              the Presentation Synthesizer.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-6">
            {filtered.map((deck) => (
              <div
                key={deck._id}
                className="bg-white border border-neutral-200 rounded-2xl p-6 shadow-sm flex flex-col md:flex-row gap-8 items-start hover:border-black transition-colors"
              >
                {/* Status / Preview */}
                <div className="w-full md:w-64 shrink-0 aspect-video bg-neutral-100 rounded-xl border border-neutral-200 overflow-hidden relative group">
                  ...
                  deck.slides[0].imageUrl !== 'error' ? (
                    <img
                      src={deck.slides[0].imageUrl}
                      alt="Cover"
                ...
                <div className="flex-1 space-y-4">
                  <div className="flex items-center gap-3 mb-2">
                    <span
                      className={`px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-md ${deck.status === 'completed' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}
                    >
                      {deck.status}
                    </span>
                  ...
                  <div className="flex gap-6 mt-6 pt-6 border-t border-neutral-100 text-sm">
                    <div className="text-neutral-500">
                      <strong className="text-black font-mono mr-2">
