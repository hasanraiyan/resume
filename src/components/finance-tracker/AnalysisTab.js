...
export default function AnalysisTab() {
  ...
  const expenseCategories = analysis.categoryBreakdown.filter((c) => c.type === 'expense');
  const incomeCategories = analysis.categoryBreakdown.filter((c) => c.type === 'income');
  const currentCategories = viewMode.includes('Expense') ? expenseCategories : incomeCategories;
  const total = viewMode.includes('Expense') ? analysis.totalExpense : analysis.totalIncome;

  // Daily flow aggregation
  const dailyData = analysis.dailyFlow.reduce((acc, d) => {
    if (!acc[d.date]) acc[d.date] = { expense: 0, income: 0 };
    acc[d.date][d.type] = d.total;
    return acc;
  }, {});
  const dailyEntries = Object.entries(dailyData).sort(([a], [b]) => a.localeCompare(b));
  const maxDaily = Math.max(...dailyEntries.map(([, v]) => v.expense), 1);

  // Account analysis aggregation
  const accountData = analysis.accountAnalysis.reduce((acc, a) => {
    if (!acc[a.accountId]) acc[a.accountId] = { name: a.name, icon: a.icon, expense: 0, income: 0 };
    acc[a.accountId][a.type] = a.total;
    return acc;
  }, {});
  const maxAccountValue = Math.max(
    ...Object.values(accountData).map((a) => Math.max(a.expense, a.income)),
    1
  );
                ...
                {currentCategories.slice(0, 5).map((cat, i) => {
                  ...
                  return (
                    <div key={cat.categoryId || i} className="flex items-center gap-2 text-xs">
                      ...
                      <span className="font-bold">{((cat.total / total) * 100).toFixed(1)}%</span>
                    </div>
                  );
                })}
              </div>
