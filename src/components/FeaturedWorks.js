    ...
    return () => {
      ...
      try {
        ScrollTrigger.getAll().forEach((trigger) => {
          if (trigger.trigger === '#featured-works') {
            trigger.kill();
          }
        });
      } catch (error) {
        ...
        <div className="relative">
          ...
          {featuredProjects.length >= 3 && (
            <>
              <button
                onClick={handlePrev}
                className="hidden md:block absolute -left-10 top-1/3 z-10 bg-white rounded-full p-3 shadow-lg hover:shadow-xl transition-all text-gray-700 hover:text-gray-900"
              >
                <ChevronLeft size={24} />
              </button>
              <button
                onClick={handleNext}
                className="hidden md:block absolute -right-10 top-1/3 z-10 bg-white rounded-full p-3 shadow-lg hover:shadow-xl transition-all text-gray-700 hover:text-gray-900"
              >
                ...
                <Link key={project.id} href={`/projects/${project.slug}`}>
                  <div className="relative group overflow-hidden rounded-2xl shadow-lg mb-8 max-w-sm cursor-pointer">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-white/70 via-white/40 to-transparent backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col justify-end items-start p-6 text-left">
                      <div className="text-xs font-semibold tracking-widest mb-2 text-gray-600">
                        {project.projectNumber} — {project.category}
                      </div>
                      <h3 className="font-semibold text-lg text-gray-800 mb-1 translate-y-3 group-hover:translate-y-0 transition-all duration-500 hover:text-gray-600 transition">
                        {project.title}
                      </h3>
