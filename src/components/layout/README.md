...
`AppLayout` is a shared layout shell component designed to standardize the structure of internal mini-apps like SnapLinks. It provides a consistent responsive layout with a desktop sidebar, a mobile bottom navigation bar (or optional hamburger menu), a unified header, and support for Floating Action Buttons (FABs).
...
| --------------------------- | ----------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `appName`                   | `string`                | **Required** | The name of the application (e.g., "SnapLinks"). Used in the logo section and fallback header title.                                                                                              |
...
| `tabTitles`                 | `Object`                | `null`       | An optional map of tab IDs to their display titles in the header. If not provided, it falls back to the tab's `label` property or the `appName`. Example: `{ chat: 'SnapLinks Chat' }`.           |
...
3.  Extract any special header buttons (like a `Settings` shortcut, or the `New Chat` button) into a `<React.Fragment>` and pass it to the `headerActions` prop.
4.  Pass the FAB (if any) to the `fab` prop.
5.  If the app hides the "Settings" tab from the mobile bottom nav (like SnapLinks does), set `hideSettingsFromMobileNav={true}`.
