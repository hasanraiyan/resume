...
const getFileIcon = (mimeType) => {
  ...
  if (mimeType.includes('zip') || mimeType.includes('tar')) return <FileArchive className="w-full h-full text-amber-500" />;
  if (mimeType.includes('javascript') || mimeType.includes('json') || mimeType.includes('html')) return <FileCode className="w-full h-full text-blue-500" />;
...
export default function FileCard({ file, viewMode }) {
    ...
    return (
      <div className="group flex items-center justify-between p-3 bg-white border border-[#e5e3d8] rounded-xl hover:border-[#1f644e] transition-colors">
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleDownload} className="p-1 hover:bg-[#fcfbf5] rounded text-[#7c8e88] hover:text-[#1f644e]">
            <Download className="w-4 h-4" />
          </button>
          <button onClick={handleToggleStar} className="p-1 hover:bg-[#fcfbf5] rounded">
            <Star className={`w-4 h-4 ${file.starred ? 'text-[#1f644e] fill-[#1f644e]' : 'text-[#7c8e88]'}`} />
          </button>
  ...
  return (
    <div className="group bg-white border border-[#e5e3d8] rounded-2xl overflow-hidden hover:border-[#1f644e] transition-all hover:shadow-sm relative">
      <div className="aspect-square bg-[#f8f9fa] flex items-center justify-center p-6 group-hover:bg-[#f0f5f2] transition-colors relative">
        <div className="w-12 h-12 text-[#7c8e88] group-hover:text-[#1f644e] transition-colors">
          {getFileIcon(file.mimeType)}
        </div>

        {file.starred && ! (file.deletedAt) && (
          <Star className="absolute top-3 right-3 w-4 h-4 text-[#1f644e] fill-[#1f644e]" />
        )}
        ...
        <div className="absolute inset-0 bg-black/5 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
           ...
           >
             <Download className="w-4 h-4 text-[#1f644e]" />
           </button>
           <ActionMenu type="file" item={file} variant="overlay" />
        </div>
      </div>

      <div className="p-3">
        <h3 className="text-sm font-bold truncate leading-tight mb-1" title={file.filename}>
          {file.filename}
        </h3>
