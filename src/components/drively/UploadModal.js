    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white w-full max-w-md rounded-3xl border border-[#e5e3d8] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="px-6 py-4 border-b border-[#e5e3d8] flex items-center justify-between bg-[#fcfbf5]">
          ...
          <button onClick={onClose} className="p-1 hover:bg-[#e5e3d8] rounded-full transition-colors">
        ...
        <div className="p-6">
          ...
          {mode === 'upload' ? (
              ...
              {selectedFiles.length > 0 && (
                <div className="max-h-40 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                   {selectedFiles.map((file, idx) => (
                     <div key={idx} className="flex items-center gap-3 p-2 bg-[#fcfbf5] rounded-lg border border-[#e5e3d8]">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-wider text-[#7c8e88]">Folder Name</label>
              <input
                type="text"
                autoFocus
                value={folderName}
        ...
        <div className="px-6 py-4 bg-[#fcfbf5] border-t border-[#e5e3d8] flex gap-3">
           ...
           <button
            ...
            disabled={isProcessing || (mode === 'upload' ? selectedFiles.length === 0 : !folderName.trim())}
           ...
           >
            {isProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : (mode === 'upload' ? 'Upload' : 'Create')}
           </button>
        </div>
      </div>
    </div>
