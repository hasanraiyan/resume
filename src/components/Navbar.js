  ...
  return (
    <>
      ...
      <nav className="hidden md:flex fixed bottom-8 left-1/2 -translate-x-1/2 z-50 items-center gap-1">
        <div className="flex items-center gap-8 px-8 py-4 glass-panel rounded-full premium-shadow transition-all duration-500 hover:scale-[1.02]">
          {/* Logo (Minimal) */}
          <Link
            href={logo.link}
          ...
          <Button
            href={cta.href}
            variant="ghost"
            className="text-xs uppercase tracking-wider font-bold ml-2 !p-0"
          >
            {cta.text}
          </Button>
        </div>
      ...
      {/* MOBILE HEADER (Top) */}
      <div className="md:hidden fixed top-0 w-full z-50 px-6 py-6 flex justify-between items-center pointer-events-none">
        <Link
          href={logo.link}
          className="font-bold text-xl pointer-events-auto glass-panel px-4 py-2 rounded-full"
        >
          {logo.text}
        </Link>
        ...
        <div className="pointer-events-auto flex gap-3">
          <button
            onClick={() => setIsSearchOpen(true)}
            className="glass-panel w-10 h-10 rounded-full flex items-center justify-center text-black"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path
                strokeLinecap="round"
          ...
          <button
            ...
            className="glass-panel w-10 h-10 rounded-full flex items-center justify-center focus:outline-none"
          >
