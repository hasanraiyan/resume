        ...
        <div className="max-w-4xl mx-auto project-detail-content">
          ...
          <div className="text-center">
            ...
            <div className="flex flex-wrap justify-center gap-2 sm:gap-3 mb-6 sm:mb-8">
              {project.tags?.slice(0, 4).map((tag, index) => (
                <Badge key={index} variant="tag">{tag.name}</Badge>
              ))}
            </div>
            <div className="flex flex-wrap justify-center gap-4">
              {project.links?.live && <Button href={project.links.live} external={true} variant="primary"><i className="fas fa-external-link-alt mr-2"></i> View Live Site</Button>}
              {project.links?.github && <Button href={project.links.github} external={true} variant="secondary"><i className="fab fa-github mr-2"></i> View Code</Button>}
              {project.links?.figma && <Button href={project.links.figma} external={true} variant="ghost"><i className="fab fa-figma mr-2"></i> Design Files</Button>}
            </div>
          </div>
          
          {project.images && project.images.length > 0 && (
            <div className="my-12 sm:my-16"><ProjectGallery images={project.images} /></div>
          )}
            ...
            <div className="lg:col-span-1">
              <div className="bg-gray-50 p-6 sm:p-8 rounded-lg sticky top-28">
                ...
                <dl className="space-y-3 text-sm">
                  {project.details?.client && <div><dt className="font-semibold text-gray-600">Client</dt><dd className="text-gray-800">{project.details.client}</dd></div>}
                  {project.details?.year && <div><dt className="font-semibold text-gray-600">Year</dt><dd className="text-gray-800">{project.details.year}</dd></div>}
                  {project.details?.duration && <div><dt className="font-semibold text-gray-600">Duration</dt><dd className="text-gray-800">{project.details.duration}</dd></div>}
                  {project.details?.role && <div><dt className="font-semibold text-gray-600">Role</dt><dd className="text-gray-800">{project.details.role}</dd></div>}
                </dl>
              </div>
            </div>
          </div>
          ...
          {project.details?.results && project.details.results.length > 0 && (
            <div className="bg-black text-white p-8 sm:p-12 rounded-lg mb-12 sm:mb-16">
              <h3 className="text-2xl sm:text-3xl font-bold mb-6 sm:mb-8 text-center">Results & Impact</h3>
              <div className="grid sm:grid-cols-2 gap-6 sm:gap-8">
                {project.details.results.map((result, index) => (
                  <div key={index} className="flex items-start"><i className="fas fa-check-circle text-green-400 text-xl mr-3 mt-1"></i><p className="text-base sm:text-lg">{result}</p></div>
                ))}
              </div>
            </div>
          )}
