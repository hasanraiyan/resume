...
function SidebarSectionBtn({ section, active, done, status, onClick }) {
  return (
    <button
      onClick={onClick}
    ...
    >
      ...
      {status && (
        <span
          className={`w-1.5 h-1.5 rounded-full shrink-0 ${active ? 'bg-white/60' : SECTION_STATUS_DOT[status] || 'bg-gray-300'}`}
        />
      )}
    </button>
  );
                      ...
                      <div className="ml-2 pl-3 border-l-2 border-[#f0f5f2] space-y-0.5 mt-0.5">
                        {modSections.map((section) => (
                          <SidebarSectionBtn
                            ...
                            status={showStatus ? section.status : null}
                            onClick={() => onNavigate(section._id)}
                          />
                        ))}
