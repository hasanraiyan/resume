...
export default function EditSectionModal({ section, onSave, onClose }) {
  ...
  const isNewSection = !section?._id;
  const [activeCreationTab, setActiveCreationTab] = useState('manual'); // 'manual' | 'ai'
  ...
  const handleSave = async () => {
    if (isNewSection && activeCreationTab === 'ai') {
  ...
  return (
    <>
      ...
      >
        ...
        {isNewSection && (
          <div
            className="flex shrink-0 bg-[#fcfbf5]"
            style={{
              borderBottom: '1px solid var(--esm-border)',
            }}
          >
            <button
              onClick={() => setActiveCreationTab('manual')}
              className="flex-1 py-3 text-center text-xs font-bold transition-all border-b-2 outline-none"
              style={{
                color: activeCreationTab === 'manual' ? 'var(--esm-ink)' : 'var(--esm-muted)',
                borderBottomColor:
                  activeCreationTab === 'manual' ? 'var(--esm-forest)' : 'transparent',
                fontFamily: 'Outfit, sans-serif',
              }}
            >
              Manual Creation
            ...
            <button
              ...
              style={{
                color: activeCreationTab === 'ai' ? 'var(--esm-forest)' : 'var(--esm-muted)',
                borderBottomColor: activeCreationTab === 'ai' ? 'var(--esm-forest)' : 'transparent',
                background: activeCreationTab === 'ai' ? 'rgba(31,100,78,0.04)' : 'transparent',
                fontFamily: 'Outfit, sans-serif',
              }}
            >
              <Sparkles className="w-3.5 h-3.5" />
              AI Section Planner
              <span className="bg-[#1f644e] text-white text-[8px] font-bold px-1 rounded-md py-0.5 ml-1">
                NEW
              </span>
            </button>
          </div>
        ...
        {(!isNewSection || activeCreationTab === 'manual') && (
          <>
            {/* ── Title Input ──────────────────────────────────────────────── */}
            <div
        ...
        >
          {isNewSection && activeCreationTab === 'ai' ? (
            <div className="space-y-4">
              <div>
                <Label>Paste Section Outline / Lecture Details</Label>
                <EsmTextarea
                  rows={14}
                  value={detail}
                  onChange={(e) => setDetail(e.target.value)}
                  placeholder={`e.g. Lecture 3: Cyclic Redundancy Check (CRC)
- Topics Covered: Cyclic Codes, CRC Mechanism, modulo-2 division, polynomials.
- Core Goal: Perform modulo-2 divisions and understand checksum generation.`}
                ...
                />
                <p className="text-[10px] text-[#7c8e88] mt-2 px-1 leading-relaxed">
                  ✨ Enter any rough lecture summary, slides notes, or section descriptions. The AI
                  will analyze the outline, generate a comprehensive summary, and structure a set of
                  **at least 5 highly specific learning goals** for this section!
                </p>
              </div>
            </div>
          ) : (
                  ...
                  <div className="flex items-center justify-between shrink-0">
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="relative" ref={modeMenuRef}>
                        ...
                        >
                          <span>
                            {modeOptions.find((m) => m.id === generationMode)?.label ||
                              generationMode}
                          </span>
                          <ChevronDown
                        ...
                        <div className="p-4 space-y-3">
                          ...
                          {block.type === 'QuizBlock' && (
                            <QuizEditor
                              ...
                              onChange={(q) =>
                                updateBlock(i, 'quiz', { ...block.quiz, questions: q })
                              }
                            />
                          )}
                                    ...
                                    >
                                      <div className="flex items-center gap-2">
                                        <input
                                          value={ds.label}
                                          onChange={(e) => {
                                            const datasets = block.chart.data.datasets.map(
                                              (d, idx) =>
                                    ...
                                    >
                                      <div className="flex items-center gap-2">
                                        <input
                                          value={item.date || ''}
                                          onChange={(e) => {
                                            const timelineItems = block.timelineItems.map(
                                              (itm, idx) =>
                                                idx === itemIdx
                ...
                <div className="space-y-6 pb-16">
                  ...
                  <div className="space-y-4">
                    ...
                    {status && (
                      <div className="flex items-center gap-2">
                        <span
                          className="esm-status-dot inline-block"
        ...
        >
          ...
          <button
            ...
            disabled={
              loading ||
              isGenerating ||
              (isNewSection && activeCreationTab === 'ai' ? !detail.trim() : !title.trim())
            }
